...
def spawn_random_car(car_list):
    new_id = (len(car_list) + 1) * random.randint(10, 100)
    choice = random.randint(1, 4)
    turn_chance = random.random() 
    fail_chance = random.random()
    priority_chance = random.random()

    is_broken = fail_chance > 0.95
    timer = random.randint(100, 400) if is_broken else 0

    # AMBULANCE LOGIC: 15% chance, but same speed as everyone else
...
def main():
    ...
    pygame.display.set_caption("Smart Adaptive Grid - STABLE VERSION")
    ...
    while True:
        ...
        try:
            client.sendall(json.dumps(cars_data).encode('utf-8'))
        ...
        screen.blit(font.render("Press SPACE to Spawn Car | R to Reset", True, WHITE), (10, 10))
        pygame.display.flip()
        clock.tick(FPS)
